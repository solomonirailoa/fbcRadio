...
function prevMusic(){
  ...
  musicIndex < 1 ? musicIndex = allStations.length : musicIndex = musicIndex;
  loadMusic(musicIndex);
  playMusic();
  playingSong(); 
}
...
function nextMusic(){
  ...
  musicIndex > allStations.length ? musicIndex = 1 : musicIndex = musicIndex;
  loadMusic(musicIndex);
  playMusic();
  playingSong(); 
}
...
mainAudio.addEventListener("ended", ()=>{
  ...
  switch(getText){
    ...
    case "repeat_one":
      ...
      break;
    case "shuffle":
      let randIndex = Math.floor((Math.random() * allStations.length) + 1); //genereting random index/numb with max range of array length
      do{
        randIndex = Math.floor((Math.random() * allStations.length) + 1);
...
for (let i = 0; i < allStations.length; i++) {
  ...
  let liAudioDuartionTag = ulTag.querySelector(`#${allStations[i].src}`);
  let liAudioTag = ulTag.querySelector(`.${allStations[i].src}`);
